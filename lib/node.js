var StateEventEmitter  = require('./util/state-eventemitter'),
    ...
    ObserveRPC         = require('./network/rpc/observe'),
    InvitationRPC      = require('./network/rpc/invitation'),
    AskRendezvousRPC   = require('./network/rpc/askrendezvous'),
...
var Node = module.exports = Peer.extend({
  ...
  initialize: function(id, options) {
    ...
    this._reactor.register({
      ...
      STORE         : StoreRPC,
      OBSERVE       : ObserveRPC,
      INVITATION    : InvitationRPC,
      ASKRENDEZVOUS : AskRendezvousRPC
    });
  ...
  join: function(callback, context) {
    ...
    var success = function() {
      this.emit('joined');
      this._chooseNextRendezvousPeer();
    };
    ...
    // pick a random 'server' node and ask it about our external addr
    var serverIdx = Math.floor(Math.random() * this._bootstraps.length);
    var server = this._bootstraps[serverIdx];
    var observe = new ObserveRPC(server);
    observe.then(function (externalAddress) {
      this.setExternalAddress(externalAddress);
    }, this);
    this._reactor.sendRPC(observe);
 ...
 //# INTERNAL EVENTS HANDLING
  ...
  /**
   * Handle an incoming OBSERVE request:
   * answer with the address of the node we see.
   *
   * @param  {ObserveRPC} rpc - the incoming rpc object
   */
  handleOBSERVE: function(rpc) {
    rpc.resolve(rpc.getQuerying().getAddress());
  },

  /**
   * Handle an incoming INVITATION request:
   * send ping response to the desired node to punch it
   *
   * @param  {InvitationRPC} rpc - the incoming rpc object
   */
  handleINVITATION: function(rpc) {
    var ping = new PingRPC(rpc.getTarget());
    this._reactor.sendRPC(ping);
    rpc.resolve();
  },

  /**
   * Handle an incoming ASKRENDEZVOUS request:
   * organize a udp hole for two nodes to be able to talk
   *
   * @param  {AskRendezvousRPC} rpc - the incoming rpc object
   */
  handleASKRENDEZVOUS: function(rpc) {
    // Ask target node to invite sender of rendezvous query
    var invitation = new InvitationRPC(rpc.getTarget(), rpc.getQuerying());
    invitation.then(function () {
      // Ok, invitation was successfull
      rpc.resolve();
    });
    this._reactor.sendRPC(invitation);
  },
 ...
 //# ITERATIVE PROCESSES
  ...
  _chooseNextRendezvousPeer: function() {
    var nodes = this._routingTable.exports();
    return nodes;
  },

  _startKeepAlive: function() {
    this._keepAliveProcess = setInterval(function(self) {
      var observe = new ObserveRPC(self.getRendezvousPeer());
      var success = function (address) {
        if (self.getExternalAddress() != address) {
          // TODO: notify about ip change
          self.setExternalAddress(address);
          // Start bootstrap to propagate node info change
          self.iterativeFindNode(self);
        }
      };
      var fail = function() {
        // TODO: find a new rendezvous peer
        // Start bootstrap to propagate node info change
        self.iterativeFindNode(self);
      }
      observe.then(success, fail);
      self._reactor.sendRPC(observe);
    }, globals.TIMEOUT_KEEP_ALIVE, this);
  },

  _stopKeepAlive: function() {
    clearInterval(this._keepAliveProcess);
  }
